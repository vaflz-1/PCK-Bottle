...
## Homebrew cask auto-bump (optional)

On each tagged release the workflow can update the cask in
[`vaflz-1/homebrew-tap`](https://github.com/vaflz-1/homebrew-tap) (version +
sha256) so `brew upgrade --cask pck-bottle` serves the new build. It runs only
when this secret is present:

| Secret | Purpose |
|--------|---------|
| `HOMEBREW_TAP_TOKEN` | a Personal Access Token with **contents: write** on the `homebrew-tap` repo |

Create a fine-grained PAT (Settings → Developer settings → Fine-grained tokens),
scope it to `homebrew-tap` with Read/Write on Contents, and add it as the secret
above. Without it, bump the cask by hand after a release.
